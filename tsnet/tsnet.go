...
type Server struct {
	...
	localAPICred        string                 // Sec-Tailscale-LocalAPI value for localAPITCPListener
...
// LocalAPI returns a loopback ip:port listening for the "LocalAPI".
//
// As the LocalAPI is powerful, access to endpoints requires passing a
// "Sec-Tailscale-LocalAPI" HTTP header containing the value of cred.
//
...
func (s *Server) LocalAPI() (addr string, cred string, err error) {
	if err := s.Start(); err != nil {
		return "", "", err
	}
	...
	if s.localAPITCPListener == nil {
		...
		go func() {
			...
			if err := http.Serve(s.localAPITCPListener, lah); err != nil {
